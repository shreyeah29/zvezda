...
export default function DomeGallery({
  ...
  const startInertia = useCallback(
    (velocityX: number, velocityY: number) => {
      const MAX_V = 1.4;
      let vX = clamp(velocityX, -MAX_V, MAX_V) * 80;
      let vY = clamp(velocityY, -MAX_V, MAX_V) * 80;
      let frames = 0;
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        startPosRef.current = { x: mouseEvent.clientX, y: mouseEvent.clientY };
      },
      onDrag: ({ event, last, velocity = [0, 0], direction = [0, 0], movement = [0, 0] }) => {
        if (focusedElRef.current || !draggingRef.current || !startPosRef.current) return;

        const mouseEvent = event as MouseEvent;
        const dxTotal = mouseEvent.clientX - startPosRef.current.x;
        const dyTotal = mouseEvent.clientY - startPosRef.current.y;
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (!overlay || !parent || !rootRef.current) return;
  ...
  const openItemFromElement = useCallback(
    (element: HTMLDivElement) => {
      ...
      if (!mainRect || !frameRect || tileRect.width <= 0 || tileRect.height <= 0 || !viewerRef.current) {
        openingRef.current = false;
        focusedElRef.current = null;
        refDiv.remove();
        unlockScroll();
      ...
      const rawSrc = parent.dataset.src || element.querySelector("img")?.getAttribute("src") || "";
